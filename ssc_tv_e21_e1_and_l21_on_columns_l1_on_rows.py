"""SSC-TV-E1E21-L21-P: hybrid L1 + L2,1 on E, L2,1 on P, L1 on Q.
...
def ssc_admm_nuc_tv_e1_e21(
    ...
    """
    SSC-ADMM with hybrid L1 + L2,1 on E, L2,1 TV on P = DC (column
    variation), and L1 TV on Q = CD^T (row variation). P is group
    soft-thresholded column-wise; Q is soft-thresholded entrywise.

    Objective
    ---------
        min   λ_e1 ||E||_1 + λ_e21 ||F||_{2,1}
              + (λ_z/2) ||Y - YX - E||_F^2 + γ(||P||_{2,1} + ||Q||_1)
    ...
    gamma     : float            TV weight (L2,1 on P, L1 on Q)
    ...
    for it in range(max_iter):
        ...
        # 3-4. P- and Q-updates
        # P: column-wise group soft threshold (L2,1 on column variation)
        # Q: entrywise soft threshold (L1 on row variation)
        DC = D @ C
        CDt = C @ D.T
