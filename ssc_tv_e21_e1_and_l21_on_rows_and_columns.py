"""SSC-TV-E1E21-L21-PQ: hybrid L1 + L2,1 on E, L2,1 on both P and Q.
...
def ssc_admm_nuc_tv_e1_e21(
    ...
    """
    SSC-ADMM with hybrid L1 + L2,1 on E and column-group (L2,1) TV on
    both P = DC and Q = CD^T. E is shrunk elementwise, then its columns
    are shrunk as groups via F; P and Q are both group soft-thresholded.

    Objective
    ---------
        min   λ_e1 ||E||_1 + λ_e21 ||F||_{2,1}
              + (λ_z/2) ||Y - YX - E||_F^2 + γ(||P||_{2,1} + ||Q||_{2,1})
    ...
    gamma     : float            L2,1 TV regularisation weight on P and Q
    ...
    for it in range(max_iter):
        ...
        # 3-4. P- and Q-updates -- column-wise group soft threshold (L2,1 TV)
        DC = D @ C
        CDt = C @ D.T
